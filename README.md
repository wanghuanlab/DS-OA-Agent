...
v1.0.3 支持同一日期添加多条任务、编辑工作描述与耗时，并可根据任务初始预计和累计消耗自动计算预计剩余。客户端还可选接入云之家个人打卡流水，按每天最早和最晚有效打卡自动计算工时。同一天包含多个禅道任务时会先平均分配，用户可以在预览中继续调整；未登录、临时凭证过期或考勤服务不可用时仅提示，不影响原有日志生成与提交。
...
3. 可选在设置中粘贴已登录的云之家个人打卡页面 URL，用于自动计算每日工时。
4. 选择填报日期和本机代码库目录，检查提交记录并选择提交人。
5. 为每个代码库关联禅道任务。
6. 生成预览，调整每条日志的任务、工作描述与耗时。
7. 点击“立即录入禅道”完成填报。
...
密码、API Key 和云之家临时授权 URL 会以本地 JSON 形式保存，请保护当前操作系统账户，并避免手动分享用户数据目录。仓库中的默认云之家地址不包含任何用户 `ticket`。
